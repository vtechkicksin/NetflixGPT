  ...
  return (
    <div>
      ...
      <form className='w-3/12 absolute p-12 bg-black/75 my-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-80'>
        <h1>Sign In</h1>
        {signUp && <input type='text' placeholder='Name' className='p-4 my-6 w-full bg-gray-700 rounded-lg'/> }
        ...
        <button className='p-4 my-6 bg-red-400 w-full rounded-lg'>Submit</button>
        <p className='py-4 cursor-pointer' onClick={()=>setSignUp(prev=>!prev)}>New to Netflix ? {signUp ? "Sign In" : "Sign Up"}</p>
      </form>
    </div>
  )
}
